...
export function registerPluginCommands(
  ...
  handler.registerCommand('plugins', {
    flags: '-',
    ...
    const lines = plugins.map((p) => `  ${p.name} v${p.version}${p.description ? ' — ' + p.description : ''}`);
    ctx.reply(`Loaded plugins (${plugins.length}):\n${lines.join('\n')}`);
  });

  handler.registerCommand('load', {
    flags: 'n',
  ...
  handler.registerCommand('unload', {
    flags: 'n',
  ...
  handler.registerCommand('reload', {
    flags: 'n',
